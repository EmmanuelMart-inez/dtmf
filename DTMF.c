...
{ int i,n,m, num_samples_rellenado;
  ...
  */
  ...
  FILE *archivoEntrada1=NULL, *archivoEscritura=NULL;
	wave wav1, wav2;
	wave *conv;	 
	archivoEntrada1=fopen("1.wav","rb");
...
*/
 ...
 // n=pow(2.0,(float)m);
  ...
  /*Decodificación de la tecla correspondiente al tono*/
  	//Barrido en frecuencia  f= 1/N y comparador
  int armonicos=0;
  int fposicion[2] = {0, 0};
  float frecuencia_fundamental[2] = {0.0, 0.0};
  for (i=0; i<n/2; i++) {
	if(armonicos < 2){
	    if(fabs(xi[i]) > 0.15){
	    	frecuencia_fundamental[armonicos] = xi[i];
	    	fposicion[armonicos] = i;
	    	armonicos++;
		}
    }
  }
  	//Calcular frecuencia nominal (col, fila)
  float per_muest_tfourier = wav1.sample_rate/nbase2;
  float teclafil = per_muest_tfourier * fposicion[0];
  float teclacol = per_muest_tfourier * fposicion[1];
  char tecla = '-';
  printf("\nMuestras: %d\n", nbase2);
  printf("\nSamplerate:%d\n", wav1.sample_rate);
  printf("\n\n Tecla fila: %f, tecla columna: %f\n", teclafil, teclacol);
  float dtmffil[4] = {697.0, 770.0, 852.0, 941.0};
  float dtmfcol[4] = {1209.0, 1336.0, 1477.0, 1633.0};

  for(i=0; i<=3; i++){
  //printf("\n%d:%f\n",i, dtmffil[i]+dtmffil[i]*0.018);
 	if( fabs(teclafil) <= (dtmffil[i]+dtmffil[i]*0.018) &&  fabs(teclafil) >= (dtmffil[i]-dtmffil[i]*0.018) ){
  		for(j=0; j<=3; j++){
  			//printf("\n%d:%f\n",j, dtmfcol[j]+dtmfcol[j]*0.018);
  			if( fabs(teclacol) <= (dtmfcol[j]+dtmfcol[j]*0.018) &&  fabs(teclacol) >= (dtmfcol[j]-dtmfcol[j]*0.018) ){
  				if(i == 0 && j == 0) tecla =  '1';
  				if(i == 0 && j == 1) tecla =  '2';
  				if(i == 0 && j == 2) tecla =  '3';
  				if(i == 0 && j == 3) tecla =  'A';
 ...
 /* if( fabs(teclafil) < (697+697*0.018) &&  fabs(teclafil) > (697-697*0.018) ){
  	  if( fabs(teclafil) < (1209+1209*0.018) &&  fabs(teclafil) > (1209-1209*0.018) ){

	  }else if( fabs(teclafil) < (770+770*0.018) &&  fabs(teclafil) > (770-770*0.018) ){
	  	
	  }else if( fabs(teclafil) < (852+852*0.018) &&  fabs(teclafil) > (852-852*0.018) ){
	  	
	  }else if( fabs(teclafil) < (941+941*0.018) &&  fabs(teclafil) > (941-941*0.018) ){
	  	
	  }else
	  	printf("\n\nNo hubo coincidencia con ninguna tecla de fila\n");
 ...
 */
  ...
  printf("\n\nDatos: %d %f\n", fposicion[0], frecuencia_fundamental[0]);
  printf("\n\nDatos: %d %f\n", fposicion[1], frecuencia_fundamental[1]);
...
*/
  ...
  for (i=0; i<N; i++) {  //Modificacion de escalamiento N -> N/N = 1 = audio scale 100%
      xr[i]=xr[i]/N;
      xi[i]=xi[i]/N;
    }
}
